...
export class StockService {
  ...
  //se usa cuando se crea una ruta y el estado pasa en envio_creado o cancelado
  ...
  async finalizarPedidoYEnvio(dto: FinalizarPedidoEnvioDto, idEmpresa: number,
    idUsuario: number) {
    ...
    try {
      ...
      const salidaGeneral = queryRunner.manager.create(SalidaStockGeneral, {
        ...
        observaciones: pedido.observaciones || 'pedido'
      });
      //obtener ids y cantidad de los productos del pedido
      ...
      const venta = queryRunner.manager.create(Venta, { // 1-inserta en tabla venta
        ...
        id_usuario: { id: idUsuario },
        tipo_venta: 'pedido', // solo venta de salón
      ...
      const envioHeader = await queryRunner.manager.findOne(EnviosHeader, {
        relations: ['envioPedido', 'envioPedido.movil'],
        where: { id: dto.id_envio, estado: 'pendiente' },
      });
      if (!envioHeader) {
        throw new Error(`No se encontró el envío con ID ${dto.id_envio}`);
      }

      //si el pedido no esta en la lista de envios retornar error
      const pedidoEnEnvio = envioHeader.envioPedido.find(ep => ep.idPedido === dto.id_pedido);
      if (!pedidoEnEnvio) {
        throw new Error(`El pedido con ID ${dto.id_pedido} no está en la lista de envíos`);
      }

      //si el movil no es el mismo que esta en el pedido retornar error
      if (pedidoEnEnvio.movil.id !== dto.id_movil) {
        throw new Error(`El pedido con ID ${dto.id_pedido} no le corresponde al móvil con ID ${dto.id_movil}`);
      }

      const movilEnPedido = envioHeader.envioPedido.some(ep => ep.idPedido === dto.id_pedido && ep.movil.id === dto.id_movil);
      if (!movilEnPedido) {
        throw new Error(`El pedido con ID ${dto.id_pedido} no le corresponde al móvil con ID ${dto.id_movil}`);
      }

      //encontar pedidos entregados, si solo falta 1 por entregar , finalizar envio
      const pedidosEntregados = await queryRunner.manager.count(Pedido,
        {
          where: { id: In(envioHeader.envioPedido.map(p => p.idPedido)), estado: 'entregado' },
        }
      );

      const difTotal: number = envioHeader.envioPedido.length - pedidosEntregados;
      if (difTotal === 1) {
        envioHeader.estado = "entregado";
        envioHeader.fechaFinalizacion = new Date();
      }

      //agregar lat y lon al envio_pedido
      const envioPedidos = await queryRunner.manager.find(EnvioPedido, {
        where: { idPedido: dto.id_pedido, envioHeader: { id: dto.id_envio } }
      });
      if (envioPedidos.length === 0) {
        throw new Error(`No se encontró el envío del pedido con ID ${dto.id_pedido}`);
      }
      //actualizar fecha entrega, lat y lon
      for (const ep of envioPedidos) {
        ep.fechaEntrega = new Date();
        ep.lat = dto.lat;
        ep.lon = dto.lon;
        await queryRunner.manager.save(ep);
      }

      //find detalle pedido y actualizar estado a entregado
      const detallesPedido = await queryRunner.manager.find(DetallePedido, {
        where: { idPedido: dto.id_pedido }
      });
      if (detallesPedido.length === 0) {
        throw new Error(`No se encontró el detalle del pedido con ID ${dto.id_pedido}`);
      }
      ...
      await queryRunner.manager.save(envioHeader);
